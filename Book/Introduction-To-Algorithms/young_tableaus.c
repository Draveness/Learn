 ...
 * =====================================================================================
 *
 *       Filename:  young_tableaus.c
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  03/19/2015 14:03:26
 *       Revision:  none
 *       Compiler:  gcc
 *
...
bool find(tableau_t *tableau, int key) {
    cell current = {tableau->m - 1, 0};

    while (within(tableau, current)) {
        int value = get(tableau, current);
        if (value == key) {
            return true;
        } else if (value > key) {
            current = up(current);
